...
class Assign:
	...
	@classmethod
	def hitung_kosong(ruang, jam_awal, jam_akhir):
		selisih = jam_akhir - jam_awal
		hitung = 0
		for i in range(selisih):
			if not self.matriks[ruang][jam_awal+i]:
				hitung += 1
		return hitung
		
	@classmethod
	def min_kosong(daftar):
		minim = 0
	...
	def cariruang(cls, matkul):
		...
		for i in range(temp_matkul_time_final.sks):
			self.matriks(temp_matkul_time_final.r_selected, c+i)
